...
def main():
    ...
    Image.fromarray((img_t * 255).astype(np.uint8)).save(f'outputs/cornell_box_GI_init.png')

    # Setup camera for training
    cam.samples_per_pixel = 1024 #1024 * 2 # 1024
    cam.max_depth         = 8 # 50
    cam.vfov = 22
    cam.lookfrom = dr.scalar.Array3f(2.65, 1.5, -8.00)
    cam.lookat = dr.scalar.Array3f(2.65, 1.5, 0)
    ...
    ref_img = Image.open('outputs/cornell_box_GI.png')
    ...
    for i in progress_bar:
        ...
        with dr.suspend_grad():
            ...
            if i % 10 == 0 or i == 1:
                ...
                Image.fromarray((output.numpy() * 255).astype(np.uint8)).save(f'outputs/cornell_box_GI_{i}.png')
    ...
    with dr.suspend_grad():
        img_t = cam.render(world)
        img_t = dr.clip(wt.linear_to_gamma(img_t), 0.0, 1.0).numpy()
    Image.fromarray((img_t * 255).astype(np.uint8)).save(f'outputs/cornell_box_GI_final.png')
    ...
    plt.ylabel('Loss')
    plt.title('Loss')
    plt.savefig('outputs/loss_GI.png')
    plt.show()
