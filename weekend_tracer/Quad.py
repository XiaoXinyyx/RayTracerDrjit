...
class Quad(Hittable):
    ...
    # Exculed by tracing
    @staticmethod
    def make_quad(Q: Array3f, u: Array3f, v: Array3f, mat: Material):
        Q = Array3f(Q)
        u = Array3f(u)
        v = Array3f(v)
        n = dr.cross(u, v)
        ...
        # Set bounding box
        bbox_diagonal1 = aabb.make_aabb(Q, Q + u + v)
        bbox_diagonal2 = aabb.make_aabb(Q + u, Q + v)
        bbox = aabb.combine(bbox_diagonal1, bbox_diagonal2)

        quad = Quad(
            Q=Q, u=u, v=v, normal=normal, D=D, w=w,
            mat_idx=UInt32(0), mat_type=UInt32(0), bbox=bbox)
        quad.material = mat
        return quad
    ...
    def translate(self, offset: Array3f):
        ...
        # Update bounding box
        bbox_diagonal1 = aabb.make_aabb(self.Q, self.Q + self.u + self.v)
        bbox_diagonal2 = aabb.make_aabb(self.Q + self.u, self.Q + self.v)
        bbox = aabb.combine(bbox_diagonal1, bbox_diagonal2)
        self.bbox = bbox
